...
/*   Updated: 2023/04/23 18:09:38 by mradwan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	v->i = 0;
	while (map->ceiling_tmp[v->i] == '0')
		v->i++;
	v->c = v->i;
	while (map->ceiling_tmp[v->i] != ',')
		v->i++;
	v->sub = ft_substr(map->ceiling_tmp, v->c, v->i++ - v->c);
	if (ft_strlen(v->sub) > 3)
		return (free(v->sub), 0);
...
int	store_the_cieling(t_cub3d *map, t_vars *v)
{
	...
	while (map->ceiling_tmp[v->i] == '0')
		v->i++;
	v->c = v->i;
	while (map->ceiling_tmp[v->i] != ',' && map->ceiling_tmp[v->i])
		v->i++;
	v->sub = ft_substr(map->ceiling_tmp, v->c, v->i - v->c);
	if (ft_strlen(v->sub) > 3)
		return (free(v->sub), 0);
